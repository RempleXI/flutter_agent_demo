...
class DocumentConverter {
  ...
  static Future<Map<String, dynamic>> preprocessFile(
      Uint8List fileBytes, FileType fileType, String fileName) async {
    print('正在分析: $fileName');
  ...
  /// 预处理Word文档
  static Future<Map<String, dynamic>> _preprocessWord(Uint8List fileBytes, String fileName) async {
    try {
      if (fileName.toLowerCase().endsWith('.docx')) {
        ...
        String textContent = '';
        if (documentXml != null) {
          final xmlContent = utf8.decode(documentXml.content as Uint8List, allowMalformed: true);
        ...
        if (coreProps != null) {
          final propsContent = utf8.decode(coreProps.content as Uint8List, allowMalformed: true);
        ...
        return {
          ...
          'wordCount': textContent.split(RegExp(r'\s+')).where((s) => s.isNotEmpty).length,
          'metadata': metadata,
        };
      } else {
  ...
  /// 预处理Excel文档
  static Future<Map<String, dynamic>> _preprocessExcel(Uint8List fileBytes, String fileName) async {
    try {
      if (fileName.toLowerCase().endsWith('.xlsx')) {
        ...
        if (sharedStringsXml != null) {
          final xmlContent = utf8.decode(sharedStringsXml.content as Uint8List, allowMalformed: true);
        ...
        if (workbookXml != null) {
          final xmlContent = utf8.decode(workbookXml.content as Uint8List, allowMalformed: true);
        ...
        if (sharedStringsText.length > 2000) {
          textPreview = '${sharedStringsText.substring(0, 2000)}'
  ...
  /// 预处理PowerPoint文档
  static Future<Map<String, dynamic>> _preprocessPowerPoint(Uint8List fileBytes, String fileName) async {
    try {
      if (fileName.toLowerCase().endsWith('.pptx')) {
        ...
        for (final file in archive) {
          if (file.name.startsWith('ppt/slides/slide') && file.name.endsWith('.xml')) {
            final xmlContent = utf8.decode(file.content as Uint8List, allowMalformed: true);
  ...
  /// 预处理PDF文档
  static Future<Map<String, dynamic>> _preprocessPdf(Uint8List fileBytes, String fileName) async {
    try {
      // 检查是否是有效的PDF文件
      ...
      try {
        ...
        if (docInfo.creationDate != null) metadata['creationDate'] = docInfo.creationDate.toString();
        if (docInfo.modificationDate != null) metadata['modificationDate'] = docInfo.modificationDate.toString();
      ...
      if (fileBytes.isNotEmpty) {
        // 取前2000字节尝试解码为文本
        final previewBytes = fileBytes.length > 2000 ? fileBytes.sublist(0, 2000) : fileBytes;
        textPreview = utf8.decode(previewBytes, allowMalformed: true);
        // 移除非打印字符
        textPreview = textPreview.replaceAll(RegExp(r'[^\x20-\x7E\x0A\x0D\t]'), '');
      }
  ...
  /// 预处理文本文件
  static Future<Map<String, dynamic>> _preprocessText(Uint8List fileBytes, String fileName) async {
    try {
      // 解码文本内容
  ...
  /// 预处理Markdown文档
  static Future<Map<String, dynamic>> _preprocessMarkdown(Uint8List fileBytes, String fileName) async {
    try {
      // 解码文本内容
      String content = utf8.decode(fileBytes, allowMalformed: true);
  ...
  static String _extractTextFromXml(String xmlContent) {
    ...
    textContent = textContent.replaceAll(RegExp('<[^>]*>', multiLine: true), '');
  ...
  static Map<String, dynamic> _extractMetadataFromXml(String xmlContent) {
    ...
    final creatorMatch = RegExp(r'<dc:creator>(.*?)</dc:creator>').firstMatch(xmlContent);
    if (creatorMatch != null) {
      metadata['creator'] = creatorMatch.group(1);
    }
    
    final createdMatch = RegExp(r'<dcterms:created[^>]*>(.*?)</dcterms:created>').firstMatch(xmlContent);
  ...
  static List<String> _extractSharedStringsFromXml(String xmlContent) {
    ...
    final matches = RegExp(r'<si><t(?:[^>]*)>(.*?)</t></si>', dotAll: true).allMatches(xmlContent);
  ...
  static Future<void> savePreprocessedResult(Map<String, dynamic> result, String outputPath) async {
    if (kIsWeb) {
