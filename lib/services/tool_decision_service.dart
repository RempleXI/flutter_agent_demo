...
class ToolDecisionService {
  ...
  static Future<ToolInfo> shouldCallTool(String userMessage) async {
    ...
    final containsNegation = negationWords.any((word) => userMessage.contains(word));
    ...
    if (matchedTools.isEmpty || matchedTools.length > 1) {
      if (matchedTools.isEmpty) {
        ...
        print('找到多个匹配项 (${matchedTools.length} 个)，使用AI决策模型: ${matchedTools.map((t) => t.specificToolName).join(', ')}');
      }
...
''';
      ...
      if (response.statusCode == 200) {
        ...
        print('AI决策模型调用失败，状态码: ${response.statusCode}，响应内容: ${response.body}，使用关键词匹配作为后备方案');
        // 如果API调用失败，回退到关键词匹配
        return analyzeMessage(userMessage);
      }
    } catch (e, stackTrace) {
  ...
  static List<ToolInfo> _findAllMatchingTools(String message) {
    ...
    // 文档处理类工具
    final formatConversionKeywords = ['格式转换', '转成', '转换为', 'pdf转', '转pdf', 'word转', '转word'];
    if (formatConversionKeywords.any((keyword) => message.contains(keyword))) {
      matchedTools.add(ToolInfo(
  ...
  /// 将AI响应映射到工具信息
  static ToolInfo _mapAiResponseToToolInfo(String aiResponse, String userMessage) {
  ...
  static ToolInfo analyzeMessage(String message) {
    ...
    final negationWords = ['不', '别', '不要', '不能', '不可以', '无需', '无须'];
    final containsNegation = negationWords.any((word) => message.contains(word));
    ...
    if (matchedTools.length > 1) {
      print('关键词匹配到多个工具: ${matchedTools.map((t) => t.specificToolName).join(', ')}');
    } else {
      print('未匹配到任何工具');
    }
